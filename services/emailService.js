...
const getFromAddress = () => {
  return process.env.EMAIL_FROM || `"Rest & Relax" <${process.env.EMAIL_USER || 'info@restandrelax.in'}>`;
};

// Create transporter - supports Resend API & Gmail fallback
...
// ==================== CONTACT US INQUIRY EMAIL ====================
export const sendContactInquiryEmail = async ({ name, email, phone, subject, message }) => {
  try {
    const transporter = createTransporter();
    const fromAddress = getFromAddress();
    const adminEmail = process.env.ADMIN_EMAIL || 'info@restandrelax.in';

    // 1. Notification email to Admin
    const adminMailOptions = {
      from: fromAddress,
      to: adminEmail,
      subject: `📩 New Contact Inquiry: ${subject || 'General Inquiry'} | Rest & Relax`,
      html: `
<!DOCTYPE html>
<html>
<head>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>New Contact Inquiry - Rest & Relax</title>
</head>
<body style="margin:0; padding:0; font-family:'Segoe UI',Arial,sans-serif; background:#f8fafc;">
    <div style="max-width:600px; margin:20px auto; background:white; border-radius:12px; overflow:hidden; box-shadow:0 10px 25px rgba(0,0,0,0.1); border:1px solid #e2e8f0;">
        <div style="background:linear-gradient(135deg,#008DDA 0%,#0278b8 100%); padding:30px; text-align:center; color:white;">
            <h2 style="margin:0; font-size:24px;">New Contact Us Inquiry</h2>
            <p style="margin:8px 0 0 0; opacity:0.9;">Received from website contact form</p>
        </div>
        <div style="padding:30px;">
            <table style="width:100%; border-collapse:collapse;">
                <tr><td style="padding:10px 0; font-weight:600; color:#374151; border-bottom:1px solid #e5e7eb;">Guest Name</td><td style="padding:10px 0; color:#1f2937; border-bottom:1px solid #e5e7eb;">${name}</td></tr>
                <tr><td style="padding:10px 0; font-weight:600; color:#374151; border-bottom:1px solid #e5e7eb;">Email Address</td><td style="padding:10px 0; color:#008DDA; border-bottom:1px solid #e5e7eb;"><a href="mailto:${email}" style="color:#008DDA;">${email}</a></td></tr>
                <tr><td style="padding:10px 0; font-weight:600; color:#374151; border-bottom:1px solid #e5e7eb;">Phone Number</td><td style="padding:10px 0; color:#1f2937; border-bottom:1px solid #e5e7eb;"><a href="tel:${phone}" style="color:#008DDA;">${phone}</a></td></tr>
                <tr><td style="padding:10px 0; font-weight:600; color:#374151; border-bottom:1px solid #e5e7eb;">Subject</td><td style="padding:10px 0; color:#1f2937; border-bottom:1px solid #e5e7eb;">${subject || 'General Inquiry'}</td></tr>
            </table>
            <div style="margin-top:25px; background:#f8fafc; border:1px solid #cbd5e1; border-radius:8px; padding:20px;">
                <h4 style="margin:0 0 10px 0; color:#1e293b; font-size:16px;">Message Content:</h4>
                <p style="margin:0; color:#475569; white-space:pre-wrap; line-height:1.6; font-size:14px;">${message}</p>
            </div>
            <div style="margin-top:25px; text-align:center;">
                <a href="mailto:${email}" style="display:inline-block; background:#008DDA; color:white; padding:12px 24px; border-radius:8px; text-decoration:none; font-weight:600;">Reply to Guest</a>
                <a href="tel:${phone}" style="display:inline-block; background:#10b981; color:white; padding:12px 24px; border-radius:8px; text-decoration:none; font-weight:600; margin-left:10px;">Call Guest</a>
            </div>
        </div>
        <div style="background:#1f2937; color:white; padding:20px; text-align:center; font-size:12px;">
            <p style="margin:0; color:#9ca3af;">Rest & Relax · Customer Inquiry Notification</p>
        </div>
    </div>
</body>
</html>
...
<head>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Inquiry Received - Rest & Relax</title>
</head>
<body style="margin:0; padding:0; font-family:'Segoe UI',Arial,sans-serif; background:#f8fafc;">
    <div style="max-width:600px; margin:20px auto; background:white; border-radius:12px; overflow:hidden; box-shadow:0 10px 25px rgba(0,0,0,0.1); border:1px solid #e2e8f0;">
        <div style="background:linear-gradient(135deg,#008DDA 0%,#0278b8 100%); padding:30px; text-align:center; color:white;">
        ...
        <div style="padding:30px; color:#334155; line-height:1.6;">
            ...
            <p>Thank you for reaching out to <strong>Rest & Relax</strong>. We have received your inquiry regarding <strong>"${subject || 'General Inquiry'}"</strong> and our team will get back to you shortly.</p>
            <div style="background:#f0fdf4; border:1px solid #bbf7d0; border-radius:8px; padding:15px; margin:20px 0;">
                <p style="margin:0; color:#166534; font-size:14px;"><strong>📞 Need immediate assistance?</strong><br>Feel free to call our helpline directly at <strong>+91 90990 48961</strong>.</p>
            </div>
            <p>Best regards,<br><strong>Rest & Relax Team</strong></p>
        </div>
        <div style="background:#1f2937; color:white; padding:20px; text-align:center; font-size:12px;">
            <p style="margin:0; color:#9ca3af;">Rest & Relax · Luxury Resort & Spa</p>
        </div>
    </div>
</body>
</html>
...
>>>>>>> 0522493 (Integrate Resend API key for booking emails and Contact Us inquiry handler)
