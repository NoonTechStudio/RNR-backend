...
y = drawTableMap(doc, y, pricingItems);
      ...
      let ty = footerY + 8;
      doc.font(FONTS.bold).text('Terms & Conditions:', 50, ty);
      doc.font(FONTS.regular);
      ty += 12;

      const tierText = CANCELLATION_TIERS
        .map(t => `${t.shortLabel} check-in: ${t.refundPercent > 0 ? `${t.refundPercent}% refund` : 'no refund'}`)
        .join('  |  ');
      doc.text(`1. Cancellation & refund (calculated on the amount paid) - ${tierText}.`, 50, ty, { width: 500 });
      ty = doc.y + 3;
      doc.text(`2. ${REFUND_TIMELINE_NOTE} To cancel, call +91 90990 48961.`, 50, ty, { width: 500 });
      ty = doc.y + 3;
      doc.text('3. Please carry a valid Govt ID proof during check-in.', 50, ty);
      ty += 12;
      doc.text('4. Remaining balance must be cleared upon arrival via Cash or UPI.', 50, ty);
      ty += 16;

      doc.fontSize(8).fillColor(COLORS.primary).text('Generated by Rest & Relax', 50, ty, { align: 'center', width: 500 });
